...
// --- Tipai tik vietiniam naudojimui (nekeičia tavo data/info.ts) ---
type HoursItem = { day: string; time: string };

type InfoView = typeof info & {
  address2?: string;
...
export default function ContactSection() {
  const v = info as InfoView;

  // Adresai žemėlapiams (antrą galima laikyti iš info.address2 jeigu turėsi)
  const address1Label = "Kalbakken Helse Hus";
  const address1 = v.address ?? "—";

  const address2Label = "Klinikk Resplandor";
  const address2 = v.address2 ?? "Solgansbrisen 5, 1364 Fornebu"; // fallback, kol nėra info.address2

  const hasAddress2 = Boolean(address2 && address2 !== "—");

  const facebookUrl =
    v.social?.facebook ?? "https://www.facebook.com/kunoirsielosterapija";
  const facebookLabel = facebookUrl.replace(/^https?:\/\/(www\.)?/, "");
              ...
              <li>
                ...
                <div className={styles.itemBody}>
                  ...
                  <a
                    ...
                    aria-label="Åpne vår Facebook-side i ny fane"
                  >
                    {facebookLabel}
                  </a>
                </div>
          ...
          <div className={styles.mapGrid} aria-label="Kart over begge adresser">
            ...
            <div className={styles.mapBox}>
              ...
              <iframe
                ...
                src={`https://www.google.com/maps?q=${encodeURIComponent(address1)}&output=embed`}
              />
            </div>
            ...
            {hasAddress2 && (
              <div className={styles.mapBox}>
                ...
                <iframe
                  ...
                  allowFullScreen
                  referrerPolicy="no-referrer-when-downgrade"
                  src={`https://www.google.com/maps?q=${encodeURIComponent(address2)}&output=embed`}
                />
              </div>
            )}
          </div>
